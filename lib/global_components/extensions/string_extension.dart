...
import 'package:stella_stays/global_components/text/text.dart';
...
extension StringExtensions on String {
  ...
  Widget dividerText({Color? color}) {
    return Row(
      children: [
        Flexible(
          child: Container(height: 1, color: color ?? SSColors.grey),
        ),
        Container(
            margin: const EdgeInsets.symmetric(horizontal: 10),
            child: text.bodySmall(color: color ?? SSColors.grey).make()),
        Flexible(
          child: Container(height: 1, color: color ?? SSColors.grey),
        ),
      ],
